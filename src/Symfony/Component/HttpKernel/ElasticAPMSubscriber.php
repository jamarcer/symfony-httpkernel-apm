...
namespace Jamarcer\APM\Symfony\Component\HttpKernel;
...
final class ElasticAPMSubscriber implements EventSubscriberInterface
{
    ...
    public function countSpans(): int
    {
    ...
    private function getCallableName($callable): string
    {
        ...
        if (true === is_array($callable)) {
            $class = is_object($callable[0]) ? get_class($callable[0]) : trim($callable[0]);
            $method = trim($callable[1]);
